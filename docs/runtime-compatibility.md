...
| headless/subagent | `ctx.ui.notify` 不可用 | 不调用 UI；可见工具文本仍为 `ok`，`details.delivered = false` 并记录 `reason` |
| Pi-family | manifest 支持 legacy `pi.extensions`，并提供兼容 extension API | 能力满足时行为同 OMP；未满足时 fail-open，诊断信息保留在 `details` |

## 已知限制

- `variant` 只支持 OMP/Pi UI notify 类型：`info`、`warning`、`error`。
- `details.notifyType` 与最终使用的 `variant` 一致；本包不做跨宿主语义转换。
- 无 UI、headless、subagent 或 `ctx.ui.notify` 缺失时，工具可见文本仍返回 `ok`，不会把 UI 不可用暴露成模型可见失败；真实状态保留在 `details.delivered = false` 与 `details.reason = "ui_unavailable"`。
- `ctx.ui.notify` 抛错或返回 rejected promise 时，工具可见文本仍返回 `ok`，并通过 logger warning 与 `details.reason = "notify_failed"` 保留诊断信息。
