...
- 使用 OMP/Pi 的 `ctx.ui.notify({ type, message })` 作为展示通道。
...
1. 若 `ctx.ui?.notify` 不是函数，不调用。
2. `ctx.hasUI` 只能作为 runtime 提示，不能作为唯一开关；RPC-like context 只要存在 `ctx.ui.notify` 就应尝试通知。
...
- interactive UI 可用时调用 `ctx.ui.notify({ type, message })`。
